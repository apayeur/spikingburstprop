...
int main(int ac, char* av[])
{
    ...
    try {
        ...
        ("binsize", po::value<float>(), "bin size for population rates")
        ;
        ...
        if (vm.count("binsize")) {
            std::cout << "bin size set to "
            << vm["binsize"].as<float>() << ".\n";
            binSize_rate = vm["binsize"].as<float>();
        }
    }
    ...
    SineCurrentInjector * sine_inject_dend1 = new SineCurrentInjector(pyr1, 0, 1./period_dend, 0., "Vd");
    SineCurrentInjector * sine_inject_soma2 = new SineCurrentInjector(pyr2, 0, 1./period, 0., "mem");
    
    // Burn-in period (i.e. relaxation) before alternating stimulation
    sys->run(simtime);
    
    // Main simulation
    sine_inject_dend1->set_amplitude(0.5*(max_dendritic_current - min_dendritic_current)/pyr1[0].get_Cd());
    sine_inject_soma2->set_amplitude(0.5*(max_somatic_current - min_somatic_current)/pyr1[0].get_Cs());
    sys->run(3*simtime);
