...
ax_raster2.eventplot(eventtimes2[:N_neurons], colors=custom_colors['blue'], lineoffsets=lineoffs, linelengths=1, lw=0.5)
ax_raster2.eventplot(bursttimes2[:N_neurons], colors=custom_colors['orange'], lineoffsets=lineoffs, linelengths=1, lw=0.5)
...
ax_raster2_tw.set_ylabel('Rate [Hz]')
...
ax_raster1.eventplot(bursttimes1[:N_neurons], colors=custom_colors['orange'], lineoffsets=lineoffs, linelengths=1, lw=0.5)
...
ax_raster1_tw.set_ylabel('Rate [Hz]')


# right column: population activities
ax_pop2_dend = fig.add_subplot(gs[0, 2])
ax_pop2_dend.plot(t, rescaled_input_pop2_dend, '--', lw=1, color='black', label='$I_d$ (rescaled)')
...
ax_pop2_dend.set_ylabel('Burst prob. [%]')
...
ax_pop2_soma.plot(t, rescaled_input_pop2_soma, '-', color=custom_colors['blue'], label='rescaled mean')
...
ax_pop2_soma.set_ylabel('Rate [Hz]')
...
ax_pop1_dend.fill_between(t, 100*(mean1['BP'] - 2*std1['BP']), 100*(mean1['BP'] + 2*std1['BP']),
                          color=custom_colors['red'], alpha=0.5, lw=0)
ax_pop1_dend.plot(t, rescaled_input_pop1_dend, ':', lw=2.5, color=custom_colors['orange'], label='rescaled mean')
ax_pop1_dend.set_xlim([pre_stim_burn, 3])
ax_pop1_dend.set_ylim([0., 65])
ax_pop1_dend.set_xticks([1, 2, 3])
ax_pop1_dend.set_ylabel('Burst prob. [%]')
...
ax_pop1_dend.annotate('c2', (-0.3, 1.05), xycoords='axes fraction')


ax_pop1_soma = fig.add_subplot(gs[3, 2])
ax_pop1_soma.plot(t, rescaled_input_pop1_soma, '--', lw=1, color='black', label='$I_s$ (rescaled)')
...
ax_pop1_soma.set_ylabel('Rate [Hz]')
